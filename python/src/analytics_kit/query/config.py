...
class QueryClientConfig(BaseModel):
    ...
    ``personal_key`` presence drives selection: unkeyed (or keyed-but-endpointless)
    configuration yields the query no-op (bar B). ``query_endpoint`` is the config-supplied
    host the HTTP adapter POSTs to; ``project_id`` scopes the query URL. ``taxonomy`` is the
    :func:`~analytics_kit.define_taxonomy` return value — an opaque, non-Pydantic object held
    via ``arbitrary_types_allowed``, so a raw dict fails here. ``transport`` is the injectable
    HTTP send hook the HTTP adapter routes through (default ``None`` — the adapter supplies a
    stdlib transport when unset; PY5-S2 wires that). Unknown keys are rejected loudly.
    """
