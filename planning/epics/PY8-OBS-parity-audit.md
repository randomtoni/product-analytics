...
- A **capability-parity matrix vs the TS surface**: every TS capability is ruled direct-analog / idiomatic-adaptation / **N-A-by-platform**, with the N-A rows (browser persistence, autocapture, pageviews, session replay, cross-subdomain cookies, sendBeacon/unload, the anon→identified merge, runtime `register`/`unregister`) **explicitly documented as by-design omissions** — no silent gap. The matrix also states the **taxonomy compile-time guarantee gap** (Python = runtime-registry parity + best-effort static, NOT TS-parity — PY3's PM-locked promise).
...
- **N-A rows are documented omissions, not gaps.** The browser-only surface (persistence, autocapture, pageviews, replay, cross-subdomain cookies, sendBeacon, merge, runtime register/unregister) is by-design-omitted server-side (the parity rule is *server-shaped*). The matrix names each explicitly so parity is auditable — the TS-E11 by-design-omitted-rows precedent (flags/replay), applied to the browser surface here.
