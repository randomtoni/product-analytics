...
- **[E17-S1](../stories/5-done/E17-S1-schema-contract-freeze.md)** *(done — `afeb15c`)* — froze `planning/WAREHOUSE-SCHEMA-CONTRACT.md` (the `events` column contract, the safe-cast typed-VIEW rule, the trait/group JSONB-nesting guard, the deterministic column order, the greenfield/wire-identity notes). The reviewed one-way-door gate.
- **[E17-S2](../stories/5-done/E17-S2-events-ddl-typed-view-generator.md)** *(done — `722d9cd`)* — library-owned `events` DDL + taxonomy-driven typed-view generator (safe-cast JSONB projections, byte-identical cross-tree) + shipped idempotent migration, TS/Python parity. The generated view requires **Postgres ≥16** (`pg_input_is_valid`) — carried to E21's recipe/acceptance test.
- **[E17-S3](../stories/5-done/E17-S3-db-execute-seam-driver-extra.md)** *(done — `a57ff5d`)* — role-named injectable DB-execute seam (mirrors the `QueryTransport`/`FetchLike` injection; no driver handle crosses it) + default `pg`/`psycopg` v3 driver behind the pinned `warehouse` extra, lazily imported; ships a reusable fake for downstream (S4/E18) tests.
- **[E17-S4](../stories/5-done/E17-S4-warehouse-dsn-config-selection-ladder.md)** *(done — `a2e2a28`)* — `warehouse_dsn` query-config field + presence-based selection ladder in `createQueryClient`/`create_query_client` (no `backend:` enum); the from-config factory builds the driver and injects the `DbExecute` into the (still-stub) `WarehouseQueryAdapter`, now config-selectable. E18 fills the method bodies.
