...
> E21 improvement pass (2026-07-14) — cosmetic (comment precision), no code/semantics change.

- Corrected the `$N`→`%s` comment's overclaim: it now says the rewrite is a no-op for `raw_query` ONLY when the raw SQL carries no `$digit` token (a `$N`-shaped literal WOULD be rewritten); the TS tree needs no such rewrite (`pg` speaks `$N` natively).
- Added a lossiness note above the `pg` `parseBigIntAsNumber` (OID 20→number is lossy past `Number.MAX_SAFE_INTEGER` — safe for the count aggregates it targets; a true `int8` via `raw_query` could exceed it; per-query opt-out is the follow-up).
- Clarified that the E1 `raw_query("SELECT count(*)::int …")` probe is a provenance anchor over OID 23 (int4), NOT the OID-20 bigint-parser exercise (the funnel/retention counts are that).
