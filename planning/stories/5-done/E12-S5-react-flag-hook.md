...
- **StrictMode `cancelled`-guard test** — the first attempt was green-but-VACUOUS (asserted on the `console.error` unmount warning that React 19 removed entirely, guarding nothing — caught by the improvement-pass reviewer, who proved removing the guard left it green). Reworked to assert on OBSERVABLE STATE: swap the provider's `flags` slot (models StrictMode's effect re-run on a mounted tree), commit a live set via `onChange`, resolve the orphaned prior `evaluate()`, then assert the committed snapshot is STILL the live set (never clobbered). Mutation-verified twice (builder + orchestrator): removing the `cancelled` guard fails it at `committed.isEnabled('live')`.
- Not done (deferred): the S1 sync `current()`-snapshot getter that would kill the first-paint flash + enable `useSyncExternalStore` — a seam-level change touching all adapters, a candidate for a future flags-hardening story, NOT an improvement-pass edit.
