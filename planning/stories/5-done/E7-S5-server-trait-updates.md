...
## Shipped
- > Reviewer verdict (2026-07-08): the untyped-escape-hatch `capture(id, 'set_traits', ...)` reserved-name collision is ACCEPTABLE R1 posture — identical in kind/severity to the shipped `MERGE_EVENT='identify'` (browser) + `AUTOCAPTURE_EVENT` risks; only fires under the no-taxonomy hatch when a consumer deliberately types a reserved internal name. NOT a ship-blocker.
- > Reviewer suggestion (2026-07-08, DEFERRED SEAM STORY — cross-adapter): the robust fix is a seam-level internal-event DISCRIMINANT (a structural `NeutralEvent` marker like `isPageView`, or a taxonomy `never`-union extension) replacing name-based recognition — retiring the `MERGE_EVENT`/`AUTOCAPTURE_EVENT`/`set_traits`/`set_group_traits` collision across BOTH browser + node at once. A SEAM change (S5/E7 forbid it) — file for a future story (surface in E11 audit or a dedicated slice). → carry to E7 improvement-pass note + roadmap awareness.
- > Reviewer suggestion (2026-07-08, improvement-pass candidate): add a one-line test documenting the known `set_traits` collision so it's not later mistaken for a regression; and `in`-guard `mapGroupProperties`'s three key reads (symmetric with `mapTraitProperties`, robust to future callers — safe today since the sole minting path always sets them).

## Shipped

> Captured by `implement-epics` on 2026-07-08. Completes the node capture surface (BRIEF §6 property-update verbs).

- **Files changed (node):** `node-analytics.ts` (+`setTraits(distinctId, traits, once?)` + `setGroupTraits(groupType, groupKey, traits)` on interface + class; both gate the RAW trait bag via SHARED `enforceAllowlist` BEFORE minting, stash under de-branded wrapper keys, enqueue an internal reserved-name `NeutralEvent` via `enqueueInternal` with `randomUUID` dedupeId; group composite distinctId `${groupType}_${groupKey}` default), `wire-mapper.ts` (+reserved `SET_TRAITS_EVENT`/`SET_GROUP_TRAITS_EVENT` names + de-branded nested wire-key constants; `mapEventToWire` keys off the reserved name → NESTED `properties.[set]`/`[set_once]` (person) + `[group_type]`/`[group_key]`/`[group_set]` (group))
- **New public API:** node `setTraits`/`setGroupTraits` (taxonomy-typed off `TX['traits']`/`TX['groups'][G]`). NO vendor/`$` on the surface. **`keyof NodeAnalytics` grew** `capture|setTraits|setGroupTraits|flush|shutdown`; seam-15 `keyof AnalyticsProvider` UNTOUCHED.
- **Wire-shape crux (node ≠ browser):** node NESTS trait bags inside `properties` (posthog-NODE `$set` convention), NOT lifted to top-level wire keys (browser merge convention). `$anon_distinct_id` omitted (no persisted anon id). Node's OWN de-branded key constants — no browser import.
- **Tests added:** node +20 (wire-mapper 7: nested-set/set-once, bags-nest-not-lift, group nested, no-`$`/no-browser-lift, reserved-name-not-conflated; adapter 11: mint-through-queue, once→set-once, dedupeId→uuid, composite distinctId, off-list throw+drop both verbs nothing-minted, wrapper-key-not-gated-raw-keys-are, traits+captures-batch-together; send-batch 2: setTraits/setGroupTraits ride real gzip transport round-trip; typing: grew node pin + `@ts-expect-error` both verbs) → 98; seam 166 unchanged
