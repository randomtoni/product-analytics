...
- **`send_cohorts` on the wire** — closed via S4: the parity suites now record the definitions GET path in the loopback's `do_GET` and assert `send_cohorts` on the ACTUAL socket request the real poller issued (over-the-wire, not the white-box `poller._url` read). Reviewer mutation-checked it (dropping `&send_cohorts=` fails the assertion).
- Not done (deferred): ETag/`If-None-Match` + poller backoff — a real poller FEATURE (conditional requests + backoff), story-optional; a future poller-hardening follow-up, NOT a sub-5-line improvement-pass edit.
