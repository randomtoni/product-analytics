...
- **`NoopAdapter.fetch` must satisfy the SPI's return type (code-shape pin):** S2 types `fetch` as `Promise<NeutralFetchResponse>`, so the no-op cannot return `void`/`undefined` — resolve a neutral empty response, e.g. `Promise.resolve({ status: 0, text: async () => '', json: async () => ({}) })` (matching S2's `NeutralFetchResponse` = `{ status; text(); json() }`). `getPersistedProperty` returns `undefined`; `getCustomUserAgent` returns `undefined`; `getLibraryId`/`getLibraryVersion` return neutral placeholder strings (they are `: string`, so `''` or a neutral constant — never a vendor token).
...
- > Reviewer suggestion (2026-07-07, improvement-pass candidate): the `config` param in `createAnalytics(config, adapter?)` is unused in the seam factory body (the two-piece shape: the target entry reads `config.key` to pick its adapter; E3+ facade concerns will consume `config` here). An unused first param reads as a smell — add a one-line comment explaining it's the seam's forward slot, unused in E2 by design. (This is exactly the CLAUDE.md-sanctioned "non-obvious gotcha" comment case.)

## Shipped

> Captured by `implement-epics` on 2026-07-07.

- **Files added:** `packages/analytics-kit/src/noop-adapter.ts` (whole-stack `NoopAdapter`), `src/create-analytics.ts` (`createAnalytics` + `AnalyticsConfig`), `src/noop-adapter.test.ts`, `src/create-analytics.test.ts`
- **Files changed:** `src/index.ts` (value exports `createAnalytics`, `NoopAdapter` + type `AnalyticsConfig`; impl still hidden)
- **New public API:** `createAnalytics(config: AnalyticsConfig, adapter?: AnalyticsAdapter): AnalyticsProvider`, `NoopAdapter`, `AnalyticsConfig` (= `{ key?: string }`)
- **Tests added:** 16 (7 noop full-SPI-silent + no-`disabled`; 9 factory: bar-B silent-when-unkeyed whole-stack, delegation-when-supplied, supplied-wins, impl-hidden, type-pins). 44 total in package.
