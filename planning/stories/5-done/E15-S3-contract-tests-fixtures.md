...
- **`ENGINE_ROW_FIELD_NAMES` near-homograph resolved (code-only).** Added `'aggregated_value'` (the trend total) to the seal list in `query-contract.fixtures.ts` so the seal loop covers it directly, and removed the now-redundant standalone `expect(...).not.toContain('aggregated_value')` line from both `http-query-adapter.test.ts` and `http-query-adapter-async.test.ts`. Verified no coverage lost — both seal tests iterate the list over fixtures that carry `aggregated_value` on the wire. No comment added (per zero-comments convention; list membership is self-explanatory). Gates green (21/21 turbo + 30/30 scan).
- **`WireRowFixture.description` — KEPT (skip-with-reason).** The field reads as intentional inline documentation of each contract fixture (the fixtures ARE the documented contract), so it aids readability rather than being dead metadata; left in place rather than dropped.
