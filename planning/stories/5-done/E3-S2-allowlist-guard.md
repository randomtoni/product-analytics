...
- > Reviewer suggestion (2026-07-07, improvement-pass candidate): the `onViolation` union is defined twice — inline on `AnalyticsConfig` (`create-analytics.ts`) and as `type ViolationPolicy` (`analytics-provider.ts`). Structurally identical so it compiles, but the story calls this union "the locked seam" — consolidate to one shared alias so the two can't drift.
- > Reviewer note (2026-07-07, DOCUMENTED LIMITATION — deliberate scope, surface to user): the guard gates **top-level keys only** (`Object.keys(bag)`), exactly per this story's scope. Consequence for the PRIVACY contract: `track('e', { user: { ssn } })` with `user` on-list passes `ssn` through nested — the value still leaves the app. By-design here, NOT an AC violation. If deep/nested-key allowlisting is wanted, it's a future design extension (a new story), not an E3 fix. Recorded so the top-level boundary is an explicit decision, not an implicit gap.

## Shipped

> Captured by `implement-epics` on 2026-07-07.

- **Files changed:** `packages/analytics-kit/src/analytics-provider.ts` (guard at the top of track/page/identify/group/setTraits + `emitViolation` typed sink + `ViolationPolicy` + 2 optional constructor params stored as `ReadonlySet<string> | undefined`), `src/create-analytics.ts` (`AnalyticsConfig.allowlist?`/`onViolation?` + threading), `src/create-analytics.test.ts` (shape-pin extended)
- **Files added:** `src/allowlist-guard.test.ts` (15 tests)
- **New public API:** `AnalyticsConfig.allowlist?: string[]`, `AnalyticsConfig.onViolation?: 'throw' | 'drop-and-error-log'` (default `throw`)
- **Tests added:** 15 (throw-by-default names key; drop-and-error-log via `console.error` spy; pre-adapter rejection; per-verb gating; identify gates BOTH bags; identity-args-not-gated; `[]`-activates vs undefined-inactive; fires-after-optOut; config threading). 81 total in package.
