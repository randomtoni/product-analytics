...
- **`send_cohorts` OVER-THE-WIRE** — reworked both trees' parity suites to record the definitions-GET path in the loopback's `do_GET`/handler and assert `send_cohorts` (+ token) on the ACTUAL socket request the real poller/adapter issued — closing the recurring S1/S3/S4 note at the socket layer (no more white-box `poller._url`). The TS `local-parity.test.ts` gained the check it lacked. Reviewer mutation-verified.
- **Clarity comment** — one line distinguishing the fallback-FIRED layer-1 ground-truth test from its zero-POST negative-control complement.
