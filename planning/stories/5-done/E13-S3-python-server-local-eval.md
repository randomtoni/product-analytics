...
- **`send_cohorts` on the wire** — closed via S4: `test_flag_parity.py` now asserts `send_cohorts` (+ token) on the ACTUAL definitions-GET socket path the real poller issued (recorded in the loopback's `do_GET`), replacing the white-box `poller._url` read. Mutation-verified.
- Not done (deliberate): the `early_exit` short-circuit remains dropped — a documented parity choice (resolved values identical to the reference; only whether a fallback fires can differ). No change.
