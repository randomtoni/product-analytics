...
- **`ENGINE_ROW_FIELD_NAMES` made an immutable `Final` tuple** (was a plain `list[str]`), mirroring the TS `as const` readonly export and preventing a test mutating the shared seal list; its seal-test consumer iterates it identically. The optional "why the `_SEAL_*_WIRE` fixtures are separate from the parity fixtures" note was skipped — the existing block comment on those fixtures already states they over-load every engine key, so a second note would be redundant. All gates green.
