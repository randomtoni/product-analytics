...
> PY3 post-close improvement pass, 2026-07-10.

- **Fixed the recipe F401** — the docstring `Protocol`+`cast` recipe imported `Analytics` (unused) alongside `create_analytics`; a consumer copy-pasting it failed their own `ruff check`, defeating the lint-clean-recipe goal. Dropped to `from analytics_kit import create_analytics`. Verified by extracting the recipe body to a scratch file and running `ruff check` → `All checks passed!` (F401 gone; no F811/F821 — the `overload`/`Literal`-from-`typing` deviation holds end-to-end). Touched only the `taxonomy.py` docstring; gates green (mypy strict 21 · ruff · pytest 134 · neutrality clean).
