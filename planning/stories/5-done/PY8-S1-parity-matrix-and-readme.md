...
- **(README) Added a one-clause note that the 6-direct/3-idiomatic split is the consumer-facing refinement of the docstring's flat "9 mapped"** (reviewer suggestion (b)) — preempts a reader diffing the README counts against `provider.py`'s docstring, which keeps the mapped nine flat. Suggestion (a) (the "const generics" phrasing) left as-is: it's consistent with `taxonomy.py:20`'s own "const-generic wall" docstring, so matching the source is intended. Gates re-verified green (pytest 427, neutrality `--full` clean — the changed README still passes its own doc-dimension scan).
