...
- `analytics_kit/noop.py`:
  - `NoopAdapter` — a null object implementing the **full** `AnalyticsAdapter` SPI (PY2-S1) as silent no-ops: `capture`/`set`/`group` do nothing; `flush`/`shutdown` return; the transport primitive returns a neutral empty response; `get_consent_state()` returns `"denied"`; the client-identity getters return neutral placeholders (never a vendor token). **Whole-stack** — nothing reaches the wire.
- `analytics_kit/factory.py`:
  - `create_analytics(config, adapter=None)` — resolves the adapter (supplied arg, else `NoopAdapter` when unkeyed/none), constructs the PY2-S2 provider around it, and returns the provider. Pydantic-parses/validates `config`. When no `key` is configured, the whole stack is the silent no-op.
...
- **`@runtime_checkable` on the SPI (coordinate with PY2-S1):** if the factory selects/asserts the adapter via `isinstance`, PY2-S1's `AnalyticsAdapter` must be `@runtime_checkable`. Prefer NOT relying on `isinstance` (a supplied-vs-none check is enough for selection) so the `Protocol` can stay plain; only mark it `@runtime_checkable` if a real need surfaces here.
- **`NoopAdapter` transport return must satisfy the SPI** (TS `E2-S4` code-shape pin): the transport primitive is typed to return a response, so the no-op returns a neutral empty response (status 0 / empty body), never `None` where a response is typed. Client-identity getters return `"analytics-kit"` / `"0.0.0"`-style neutral placeholders, never a vendor token.
