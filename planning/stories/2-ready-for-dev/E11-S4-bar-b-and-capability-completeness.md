...
- **Capability-completeness check** = a prose coverage table + a gated export/type-presence assertion (architect-locked shape — see Technical notes):
  - **Prose coverage table:** one row per BRIEF §Capability-contract line → the shipped neutral export/module that realizes it → the posthog-js reference it maps to (by role; this table lives in `planning/`/audit doc, exempt, so reference citations are fine here). Covers the 15-verb client surface, the implicit capabilities (anon identity + persistence, enrichment, transport/reliability, autocapture), the 3 node verbs, the 5 query methods — AND a "typed extension point, not implemented — by design" row for flags/replay/surveys/heatmaps citing BRIEF §"Explicitly OUT". Scoped to the BRIEF contract, NOT everything PostHog ships.
  - **Gated export/type-presence assertion:** a `typecheck`-time (and/or `dist`-runtime) check that the frozen 15-verb `AnalyticsProvider` surface + the 3 node verbs + the 5 query methods are ACTUALLY present/shaped on the built exports — the staleness tripwire that keeps the prose table honest.
...
- [ ] A GATED assertion proves every frozen-15 verb (13 methods + `flags?`/`replay?`), every node verb (`capture`/`setTraits`/`setGroupTraits`), and every query method (`funnel`/`retention`/`trend`/`uniqueCount`/`rawQuery`) is present on the BUILT `dist` exports and correctly shaped — a rename/drop fails `typecheck` (or the `dist`-presence test).
...
- **Capability-completeness shape — architect-locked (2026-07-09):** primarily a PROSE coverage table (the "nothing is lost" claim is a judgment mapping — no assertion can decide semantic equivalence), PLUS a cheap high-value gated slice: an export/type-presence assertion over the built `dist`. Freeze the contract as a literal list (15 client verbs + 3 node verbs + 5 query methods), import from each package's `dist` entry (not `src`, so it asserts what tsup actually ships), and assert presence + shape. Prefer a `satisfies`/conditional-type assertion at `typecheck` time (catches rename/signature drift, strongest + cheapest) complemented by `dist`-runtime presence (proves tsup emitted them). Do NOT try to gate meaning — that's the table's + reviewer's job; conflating them yields a brittle test that fails on cosmetics and still can't prove the claim.
