...
// A fake Express req carrying the RAW body Buffer (as `express.raw()` yields it) + headers.
function fakeRequest(body: Buffer, headers: ReceiverHeaders): ExpressRequestLike {
  return { body, headers };
}

// A fake Express res recording the status and whether `end()` was called (chainable `status`).
function fakeResponse(): ExpressResponseLike & { code: number; ended: boolean } {
  const res = {
    code: 0,
    ended: false,
    status(code: number) {
...
test('reads the raw body + headers and responds 200 on an accepted envelope', async () => {
  const fake = createFakeDbExecute();
  const handler = createExpressReceiver(createReceiver(fake.execute));
...
test('passes the raw body + headers verbatim to the S1 core (thin wrapper)', async () => {
  const receive = vi.fn().mockResolvedValue({ outcome: 'accepted', accepted: 1 });
  const receiver: Receiver = { receive };
  const handler = createExpressReceiver(receiver);
...
test('a DB write failure maps to a neutral 500 and leaks no driver message', async () => {
  const driverError = new Error('ECONNREFUSED 10.0.0.5:5432 — role "app" does not exist');
